...
class GeometryAttentionLayer(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.q_proj = nn.Linear(d_model, d_model, bias=False) # Linear(528,528)
    ...
    def forward(self, x, source, x_pe, source_pe, x_mask=None, source_mask=None):
        ...
        # attention
        a = torch.einsum("nlhd,nshd->nlsh", qw, kw) # [bs, nt, ns, h] h is num_head
        if kv_mask is not None:
            a.masked_fill_( q_mask[:, :, None, None] * (~kv_mask[:, None, :, None]), float('-inf')) # nothing changed here
        a =  a / qw.size(3) **0.5
        a = torch.softmax(a, dim=2)
        o = torch.einsum("nlsh,nshd->nlhd", a, vw).contiguous()  # [N, L, (H, D)] [bs, nt, h, d/h]---> [bs, 1264, 4, 132]
        ...
        e = x + message # [bs, nt, d] ---> [1, 1264, 528]
...
class RepositioningTransformer(nn.Module):
    ...
    def forward(self, src_feat, tgt_feat, s_pcd, t_pcd, src_mask, tgt_mask, data, T = None, timers = None):
        '''
        src_feats: [bs, ns, d] source feature
        tgt_feats: [bs, nt, d] target feature
        s_pcd: [bs, ns, 3] source point cloud
        t_pcd: [bs, nt, 3] target point cloud
        src_mask: [bs, ns] all True
        tgt_mask: [bs, nt] all True
        ...
        if T is not None: # second TMP
            R, t = T
            src_pcd_wrapped = (torch.matmul(R, s_pcd.transpose(1, 2)) + t).transpose(1, 2) # align source with the coarse [R|t]
            tgt_pcd_wrapped = t_pcd
        else: # first TMP
            src_pcd_wrapped = s_pcd # [bs, n_src, 3]
            tgt_pcd_wrapped = t_pcd # [bs, n_tgt, 3]

        src_pe = self.positional_encoding( src_pcd_wrapped) # [bs, n_src, dim_feat (528), 2]
        tgt_pe = self.positional_encoding( tgt_pcd_wrapped) # [bs, n_tgt, dim_feat (528), 2]
        ...
        if not self.entangled:
            ...
            src_feat = VolPE.embed_pos(self.pe_type, src_feat, src_pe) # put position encoding into features
            tgt_feat = VolPE.embed_pos(self.pe_type, tgt_feat, tgt_pe)

            for layer, name in zip(self.layers, self.layer_types):
                # self.layer_types: ['self', 'cross', 'positioning', 'self', 'cross']
                if name == 'self':
                    if self.timers: self.timers.tic('self atten')
