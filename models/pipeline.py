...
class Pipeline(nn.Module):
    ...
    def forward(self, data,  timers=None):
        ...
        coarse_feats = self.backbone(data, phase="coarse")# [n_src + n_tgt, 528]------[2146,528]
        ...
        '''
        src_feat:  [bs, n_src, 528]
        tgt_feats: [bs, n_tgt, 528]
        s_pcd:     [bs, n_src, 3]
        t_pcd:     [bs, n_tgt, 3]
        src_mask:  [bs, n_src]
        tgt_mask:  [bs, n_tgt]
        '''
        data.update({ 's_pcd': s_pcd, 't_pcd': t_pcd }) # no key words s_pcd and t_pcd before this step
        ...
        R, t, _, _, _, _ = self.soft_procrustes(conf_matrix_pred, s_pcd, t_pcd, src_mask, tgt_mask)# R: [3,3]; t: [3,1]
