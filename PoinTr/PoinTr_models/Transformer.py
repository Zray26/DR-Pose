...
class PCTransformer(nn.Module):
    ...
    def __init__(self, in_chans=3, embed_dim=768, depth=[6, 6], num_heads=6, mlp_ratio=2., qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
        ...
        config.vol_bnds = [ [ -3.6, -2.4,  1.14], [ 1.093, 0.78, 2.92 ]]
        config.voxel_size = 0.04
        config.pe_type = 'rotary'
    ...
    def forward(self, inpc):
        ...
        pe = self.positional_encoding(coor.permute(0,2,1))
        p_cos, p_sin = pe[...,0] ,pe[...,1]
        ...
        for i, blk in enumerate(self.encoder):
            if i < self.knn_layer:
                x = VolPE.embed_rotary(x, p_cos, p_sin)
